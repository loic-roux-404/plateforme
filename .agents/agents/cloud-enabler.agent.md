...
3. Load the matching skill: `terraform-engineer`, `terragrunt-platform`, `libvirt`, or `sops-secrets-platform`.
4. Plan with `terragrunt plan` (or `terraform plan` inside the module).
5. Implement the smallest coherent change.
6. Validate with `terragrunt validate`, `terraform plan`, and the layer-specific command from `AGENTS.md`.
...
- For Contabo: provider status/API check via `cntb` or web console
