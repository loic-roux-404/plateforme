...
Login to the cluster with Dex OIDC. The command is generated by Terraform (`oidc_login_setup_command_ops` output in `tf-root-paas`) and aliased in the Makefile:

```bash
make login                 # local environment
make login ENV=contabo     # production
```

This configures your kubeconfig with:

- the `oidc` user — `kubectl oidc-login` exec plugin against `https://dex.<paas_base_domain>` (scopes `groups` + `email`)
- the `plateforme` cluster — `https://<paas_base_domain>:6443` with the cluster CA fetched from the node
- the `plateforme` context — user `oidc` + cluster `plateforme`, activated

> Requires the paas layer to be applied (`make terragrunt/paas/<env>`).
